...
CAStarHeapInteger::CAStarHeapInteger()
{
	Setup=NULL;
	airoute=NULL;
}

CAStarHeapInteger::CAStarHeapInteger(CSetup *Setup, AIROUTE *airoute)
{
	this->Setup=Setup;
...
void CAStarHeapInteger::Reset(AIROUTE *airoute)
{
	//
	bigtick.QuadPart=0;
	LARGE_INTEGER tmp1;
...
inline int CAStarHeapInteger::LEFT(int k)
{
	return k<<1; //k*2;
}
inline int CAStarHeapInteger::RIGHT(int k)
{
	return (k<<1)+1; //k*2+1;
}
inline int CAStarHeapInteger::PARENT(int k)
{
	return (k>>1); //k/2;
}
inline bool CAStarHeapInteger::NOTEMPTY_UP(int k)
{
	return k!=0;
}
inline bool CAStarHeapInteger::NOTEMPTY_DOWN(int k)
{
	return k<=last_heap_leaf;
}
inline void CAStarHeapInteger::swap_heap(const int k1, const int k2)
{
	WORD tmp=heap[k1];
	heap[k1]=heap[k2];
	heap[k2]=tmp;
...
//
void CAStarHeapInteger::remove_root_from_heap()
{
...
//
void CAStarHeapInteger::insert_node_to_heap(WORD node)
{
...
//
void CAStarHeapInteger::FindPath()
{
	if(pathing_state)
		return;
	if(
...
//
void CAStarHeapInteger::without_terrain()
{
...
//
void CAStarHeapInteger::with_terrain()
{
	int count=iterations_per_frame;
	do
	{
...
// the test machine. :)
inline WORD CAStarHeapInteger::distance(const WORD yx)
{
	...
	case PYTHAGORAS_DISTANCE: //aka STRAIGHT LINE, pythagoras
		{
			...
			return (WORD)(float)sqrt((float)(xpart*xpart + ypart*ypart));
		}
		break;
...
void CAStarHeapInteger::GetOptions()
{
...
void CAStarHeapInteger::UpdateWorld()
{
	LARGE_INTEGER tmp1;
	QueryPerformanceCounter(&tmp1);
...
void CAStarHeapInteger::UpdateSettings()
{
...
///////////////////////////////////////////////////////////////////////////
// TYPE B
void CAStarHeapInteger::PackRoute()
{
...
*/
//
void CAStarHeapInteger::Paint(LPBYTE pwBits, HDC hdc,HWND hWnd)
{
	if(pwBits)
	{
		//
		...
		//		_RGBA *pend=(_RGBA *)pwBits+clientwidth*clientheight;
		...
		for(node=1;node<=free_node;node++)
		{
			...
			WORD h=nodes[node].f-nodes[node].g;
		...
		for(node=1;node<=free_node;node++)
		{
			...
			tmp->red=(BYTE)(((float)nodes[node].f/(float)largest_f)*255.0f);
			tmp->green=0;
			tmp->blue=0;
		};
		for(n=1;n<=last_heap_leaf;n++)
		{
			...
			tmp->red=0;
			tmp->green=0;
			tmp->blue=(BYTE)(((float)nodes[node].f/(float)largest_f)*255.0f);
		}
		...
		for(n=1;n<=last_heap_leaf;n++)
		{
			...
			BYTE f=(BYTE)(((float)nodes[node].f/(float)largest_f)*255.0f);
			BYTE g=(BYTE)(((float)nodes[node].g/(float)largest_g)*255.0f);
			BYTE h=(BYTE)(((float)(nodes[node].f-(float)nodes[node].g)/largest_h)*255.0f);
			tmp=(p+(nodes[node].yx>>YSHIFT)*clientwidth+(nodes[node].yx&XMASK));
			tmp->red=h;
			tmp->green=g;
			tmp->blue=f;
		...
		for(node=1;node<=free_node;node++)
		{
			BYTE f=(BYTE)(((float)nodes[node].f/(float)largest_f)*255.0f);
			BYTE g=(BYTE)(((float)nodes[node].g/(float)largest_g)*255.0f);
			BYTE h=(BYTE)(((float)(nodes[node].f-nodes[node].g)/(float)largest_h)*239.0f);
			tmp=(p+(nodes[node].yx>>YSHIFT)*clientwidth+(nodes[node].yx&XMASK));
			tmp->red=h;
			tmp->green=g;
			tmp->blue=f;
