// DepthFirst.cpp: implementation of the DepthFirst class.
//
//////////////////////////////////////////////////////////////////////
...
CDepthFirst::CDepthFirst()
{
	Setup=NULL;
	airoute=NULL;
}

CDepthFirst::CDepthFirst(CSetup *Setup, AIROUTE *airoute)
{
	this->Setup=Setup;
...
void CDepthFirst::Reset(AIROUTE *airoute)
{
	//
	bigtick.QuadPart=0;
	LARGE_INTEGER tmp1;
...
void CDepthFirst::FindPath()
{
	...
	//
	frame+=FindPath2(iterations_per_frame);
...
//
int CDepthFirst::FindPath2(int iterations_per_frame)
{
	//
	...
	do
	{
		...
		/////////////
		world[ nodes[last_node].yx ].state=CLOSED;
		WORD parentyx=nodes[last_node].yx;
		if(endyx==parentyx)
			path_found=true;
		else
		{
			//if(--last_node>=MAX_OPEN_NODES) first_node=1; //
			--last_node;
			closed_nodes++; //
			--open_nodes;
		}
	...
	//
	return (iterations_per_frame-count);
}
...
void CDepthFirst::UpdateSettings()
{
...
*/
//
void CDepthFirst::Paint(LPBYTE pwBits, HDC hdc,HWND hWnd)
{
	if(pwBits)
	{
		//
		...
		for(y=0;y<HEIGHT;y++)
		{
			for(x=0;x<WIDTH;x++)
			{
				...
				tmp->red=(BYTE)((world[yx].state==UNKNOWN)?255:0);
			}
		}
