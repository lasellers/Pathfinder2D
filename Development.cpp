...
CDevelopment::CDevelopment(CSetup *Setup, AIROUTE *airoute)
{
	this->Setup=Setup;
...
void CDevelopment::Reset(AIROUTE *airoute)
{
	//
	bigtick.QuadPart=0;
	LARGE_INTEGER tmp1;
...
inline int CDevelopment::LEFT(int k)
{
	return k<<1; //k*2;
}
inline int CDevelopment::RIGHT(int k)
{
	return (k<<1)+1; //k*2+1;
}
inline int CDevelopment::PARENT(int k)
{
	return (k>>1); //k/2;
}
inline bool CDevelopment::NOTEMPTY_UP(int k)
{
	return k!=0;
}
inline bool CDevelopment::NOTEMPTY_DOWN(int k)
{
	return k<=last_heap_leaf;
}
inline void CDevelopment::swap_heap(const int k1, const int k2)
...
inline void CDevelopment::remove_root_from_heap()
{
...
inline void CDevelopment::insert_node_to_heap(const WORD node, const WORD g)
{
...
//
void CDevelopment::without_terrain()
{
...
//
void CDevelopment::with_terrain()
{
	int count=iterations_per_frame;
	do
	{
...
// manhattan
inline WORD CDevelopment::distance(const WORD yx)
{
...
void CDevelopment::UpdateSettings()
{
...
*/
//
void CDevelopment::Paint(LPBYTE pwBits, HDC hdc,HWND hWnd)
{
	if(pwBits)
	{
		//
