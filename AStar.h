...
public:
	CAStar();
	virtual ~CAStar();
	CAStar(CSetup *Setup, AIROUTE *airoute);
	void PackRoute();
	...
	enum
	{
		...
		MAX_HEAP_LEAFS=4096, //3072, //4096, // 8192,
		NO_ROUTE=8
	};
	enum
	{
...
public:
	/*
	NOTE:
	Data structures here (and in other algs) have been put in an order that attempts to
	minimize cache miss and virtual disk page-in penalties.
	*/
